...
# Total max wait: 60 × 1s = 60 seconds before timeout error is raised.
_MAX_ARMING_RETRIES = 60
_ARMING_RETRY_WAIT  = 1.0   # seconds between arming retries
...
class AjaxAPI:
    ...
    async def _arm_command_with_retry(self, hub_id: str, command: str) -> None:
        ...
        Retry policy (max _MAX_ARMING_RETRIES = 60, wait _ARMING_RETRY_WAIT = 1s):
          - HTTP 200/204          → success, return immediately.
          - HTTP 429 / rate-limit → wait 1s and retry.
          - HTTP 5xx server error → wait 1s and retry.
          - Network timeout/error → wait 1s and retry.
          - HTTP 4xx (non-429)    → not retriable, raise AjaxAPIError immediately.
          - Exceeded 60 retries   → raise AjaxAPIError (operation timed out).
        """
        _TIMEOUT_MSG = (
            f"'{command}' command timed out — the Ajax server did not confirm the operation "
            f"after {_MAX_ARMING_RETRIES} attempts ({_MAX_ARMING_RETRIES}s). "
            "Please verify the hub status in the Ajax app and try again."
        )
            ...
            try:
                ...
                if self._detect_rate_limit(status, result):
                    if attempt < _MAX_ARMING_RETRIES:
                        _LOGGER.warning(
                            "arm %s hub %s: rate limited (attempt %d/%d) — retrying in %.0fs",
                            command, hub_id, attempt + 1, _MAX_ARMING_RETRIES, _ARMING_RETRY_WAIT,
                        )
                        await asyncio.sleep(_ARMING_RETRY_WAIT)
                        continue
                    _LOGGER.error("arm %s hub %s: %s", command, hub_id, _TIMEOUT_MSG)
                    raise AjaxAPIError(_TIMEOUT_MSG)

                if status >= 500:
                    if attempt < _MAX_ARMING_RETRIES:
                        _LOGGER.warning(
                            "arm %s hub %s: server error HTTP %d (attempt %d/%d) — retrying in %.0fs",
                ...
                if attempt < _MAX_ARMING_RETRIES:
                    _LOGGER.warning(
                        "arm %s hub %s: network error (attempt %d/%d): %s — retrying in %.0fs",
                        command, hub_id, attempt + 1, _MAX_ARMING_RETRIES, exc, _ARMING_RETRY_WAIT,
                    )
                    await asyncio.sleep(_ARMING_RETRY_WAIT)
                else:
                    _LOGGER.error("arm %s hub %s: %s", command, hub_id, _TIMEOUT_MSG)
                    raise AjaxAPIError(_TIMEOUT_MSG) from exc
